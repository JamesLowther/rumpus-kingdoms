...
async def handle_no_kingdom(ctx):
    await ctx.channel.send("Please use the command: `" + cfg.PREFIX + "init <kingdom name>` to create a new kingdom.")
...
async def rename_kingdom(ctx, *args):
    ...
    if len(args) == 0:
        await ctx.channel.send("Please use the commnd: `" + cfg.PREFIX + "rename <new name>` to rename your kingdom.")
        return

    new_name = args[0]

    await ctx.channel.send("Are you sure you would like to name your kingdom '" + new_name + "'? (y/n)")
    ...
    cfg.db_cur.execute("UPDATE Kingdoms SET k_name=? WHERE uid=?;", (new_name, str(ctx.author.id)))
    cfg.db_conn.commit()
